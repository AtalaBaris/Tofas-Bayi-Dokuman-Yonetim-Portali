...
## Şu anki iskelet durumu (2026-07-16 güncellemesi — kod yokmuş gibi varsayıp yeniden yazmayın, ama var sanıp da güvenmeyin)
...
- **Materials backend'i yazıldı, PR #3 ile `Develop`'a merge bekliyor** (`feature-backend-materials`): `MaterialsController` (`api/materials` — sınıf seviyesinde `[Authorize]` var, yani list/get/download **de** kimlik doğrulama ister, giriş yapılmamışsa 401 döner; create/update/archive ayrıca `[Authorize(Roles = "Admin,ContentManager")]` ile kısıtlı), `MaterialService`, `MaterialRepository`. Kritik marka-eşleşme kuralı (`DealerBrands ∩ MaterialBrands ≠ ∅`) `GetAuthorizedMaterialAsync`'de uygulandı: `DealerUser` rolü için marka eşleşmiyorsa veya materyal `Active` değilse `ForbiddenAccessException` (→ 403) fırlatılır, response body'de içerik hiç dönmez; `Admin`/`ContentManager` bu kısıtlamadan muaftır. `TODO.md`'deki ayrı `5. feature-*-bayi-marka-erisimi` maddesi bu PR ile fiilen tamamlandı, ayrı bir dal gerekmeyebilir (ekiple teyit edin). `MaterialStatus` enum'u artık `Material.Status`'e bağlı (önceden tanımlı ama kullanılmıyordu).
- **Backend'de hâlâ yazılmayanlar**: `BrandsController`/`DealersController`/`CategoriesController`/`UsersController` (Tanım Yönetimi, `TODO.md` madde 3), `AccessLogsController` + VIEW/DOWNLOAD loglama (`TODO.md` madde 6 — bir takım arkadaşı `feature-backend-girisLog` dalında bunun üzerinde çalışıyor olabilir, başlamadan önce o dalı kontrol edin).
- **Frontend'de `features/admin/shared-docs-list-page/`** (PR #2 ile merge oldu) gerçek ve iyi kalitede bir doküman listesi ekranı, ama **hâlâ 60 satırlık mock veri** (`MOCK_DOCUMENTS`) üzerinde çalışıyor — yukarıdaki gerçek `MaterialsController`'a henüz bağlanmadı. Eski `features/materials/` altındaki `material-list`/`material-detail`/`material-form` bileşenleri ise tamamen boş stub olarak kalmaya devam ediyor.
- EF Core migration'ı hâlâ tek: `InitialCreate` (Materials backend'i mevcut şemayı değiştirmeden, sadece `Status` kolonunu C# tarafında enum'a çevirerek eklendi — yeni migration gerekmedi).
