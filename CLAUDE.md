...
- **Materials backend'i yazıldı, PR #3 ile `Develop`'a merge bekliyor** (`feature-backend-materials`): `MaterialsController` (`api/materials` — list/get/download herkese açık, create/update/archive yalnızca Admin/ContentManager), `MaterialService`, `MaterialRepository`. Kritik marka-eşleşme kuralı (`DealerBrands ∩ MaterialBrands ≠ ∅`) burada uygulandı — `TODO.md`'deki ayrı `5. feature-*-bayi-marka-erisimi` maddesi bu PR ile fiilen tamamlandı, ayrı bir dal gerekmeyebilir (ekiple teyit edin). `MaterialStatus` enum'u artık `Material.Status`'e bağlı (önceden tanımlı ama kullanılmıyordu).
